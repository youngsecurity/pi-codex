...
async function withAppServer(cwd, fn) {
  ...
  try {
    ...
    const isBrokerBusy = client?.transport === "broker" && error?.rpcCode === BROKER_BUSY_RPC_CODE;
    // Direct fallback is only safe when the broker endpoint is dead before any
    // request is accepted. BROKER_BUSY means another stream owns the runtime;
    // starting a parallel direct app-server would defeat the broker's
    // single-owner guarantee and risk concurrent edits to the same worktree.
    const shouldRetryDirect =
      brokerRequested && !isBrokerBusy && (error?.code === "ENOENT" || error?.code === "ECONNREFUSED");
    ...
    if (isBrokerBusy) {
      const busyError = new Error(
        "Codex shared runtime is busy with another task. Wait for the active task to finish or run /codex:cancel <id>."
      );
      busyError.code = "CODEX_BROKER_BUSY";
      throw busyError;
    }
